...
public class HapticGrabber : MonoBehaviour 
{
	...
	private void hapticTouchEvent( bool isTouch )
	{
		...
		{
			if (isTouch)
				hapticDevice.GetComponent<HapticPlugin>().PhysicsManipulationEnabled = true; //La physique commence ici, turn it off maybe? Or find where the physics is acutally processed
			else			
